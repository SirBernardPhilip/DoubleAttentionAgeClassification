...
class Trainer:
    ...
    def __init__(self, params, device):
        ...
        self.w = torch.tensor(0.5, requires_grad=True).to(self.device)
    ...
    def train(self):
        ...
        for self.epoch in range(self.starting_epoch, self.params.max_epochs):  # loop over the dataset multiple times
            ...
            for input, label in self.training_generator:
                ...
                loss1 = self.criterion(AMPrediction, label)
                loss2 = self.criterion2(prediction, label)
                
                loss = (1-self.w)*loss1+self.w*loss2
                loss.backward()
